...
###### 实验中通过对参数的运行，例如model，一开始没有真正理解清楚模块参数变化带来的特征图尺寸或者是通道数的变化情况，通过查阅资料现已经理解清楚。
###### 实验中还遇到训练效果不好的问题，上网查阅资料发现Inception预训练模型网络结构很深，所以少量数据及可能不足以支撑实验效果，通过改进实验数据量大小，改善了损失和准确率的大小。
##
